 ...
 */
class INET_API PositionTableL {
    private:
        typedef std::pair<simtime_t, Coord> AddressToPositionMapValue;
        typedef std::map<int, AddressToPositionMapValue> AddressToPositionMap;
    ...
    public:
        ...
        bool hasPosition(const int & address) const;
        Coord getPosition(const int & address) const;
        void setPosition(const int & address, const Coord & coord);

        void removePosition(const int & address);
