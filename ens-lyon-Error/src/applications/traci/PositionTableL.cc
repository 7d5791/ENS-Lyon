...
#include "PositionTableL.h"

static double const NaN = 0.0 / 0.0;

std::vector<int> PositionTableL::getAddresses() const {
...
bool PositionTableL::hasPosition(const int & address) const {
    AddressToPositionMap::const_iterator it = addressToPositionMap.find(address);
    return it != addressToPositionMap.end();
}

Coord PositionTableL::getPosition(const int & address) const {
    AddressToPositionMap::const_iterator it = addressToPositionMap.find(address);
...
void PositionTableL::clear() {
    addressToPositionMap.clear();
}

simtime_t PositionTableL::getOldestPosition() const {
